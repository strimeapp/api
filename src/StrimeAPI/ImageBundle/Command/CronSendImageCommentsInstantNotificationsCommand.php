...
namespace StrimeAPI\ImageBundle\Command;
...
class CronSendImageCommentsInstantNotificationsCommand extends ContainerAwareCommand
{
    protected function configure()
    {
        $this
            ->setName('cron:image:send-comments-instant-notifications')
            ->setDescription('Send the instant notifications for the image comments')
        ;
    }

    protected function execute(InputInterface $input, OutputInterface $output)
    {
        $output->writeln( "[".date("Y-m-d H:i:s")."] CRON which sends instant notifications for the image comments" );
        ...
        if($comments != NULL) {
            ...
            foreach ($comments as $comment) {
                ...
                // And if the owner of the image wants to be notified right away
                if((strcmp($comment->getImage()->getUser()->getMailNotification(), "now") == 0)
                    && (($comment->getUser() == NULL) || ($comment->getUser()->getId() != $comment->getImage()->getUser()->getId()))) {
                    ...
                    // If the owner of the image has chosen to receive the notifications right away
                    // Add it to the list
                    $notifications[ $comment->getImage()->getUser()->getSecretId() ][] = $comment->getSecretId();
                }
            ...
            foreach ($notifications as $user_id => $comments_list) {
                ...
                foreach ($comments_list as $comment) {
                    ...
                    // Set the endpoint
                    $endpoint = $strime_app_url."app/webhook/send-comments-right-away/".$user_id."/image/".$comment;
