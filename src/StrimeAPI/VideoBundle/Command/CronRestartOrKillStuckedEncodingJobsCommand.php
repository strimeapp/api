...
class CronRestartOrKillStuckedEncodingJobsCommand extends ContainerAwareCommand
{
    protected function configure()
    {
        $this
            ->setName('cron:encoding-job:restart-or-kill-if-stucked')
    ...
    protected function execute(InputInterface $input, OutputInterface $output)
    {
        ...
        $query->select( 'api_encoding_job' );
        $query->from( 'StrimeAPIVideoBundle:EncodingJob','api_encoding_job' );
        $query->where( 'api_encoding_job.restart_time < :restart_time' );
        $query->andWhere( 'api_encoding_job.status = :status' );
        $query->andWhere( 'api_encoding_job.started = :started' );
        ...
        if($encoding_jobs != NULL) {
            ...
            foreach ($encoding_jobs as $job) {
                ...
                // Else, if the encoding job has already been restarted once, we simply kill it.
                    ...
                    $encoding_job_time = $em->getRepository('StrimeAPIStatsBundle:EncodingJobTime')->findOneBy(array('video' => $job->getVideo()));
                    ...
                    $output->writeln( "[".date("Y-m-d H:i:s")."] - delete video" );
                    ...
                    // Set the endpoint
                    $endpoint = $strime_api_url."video/".$job->getVideo()->getSecretId()."/delete";
                    ...
                    // If the video has been properly deleted
                    if($curl_status == 204) {
                    ...
                    // If an error occured in deleting the video
                    else {
                        $output->writeln( "[".date("Y-m-d H:i:s")."] - ERROR: video not deleted (cURL status: " . $curl_status . ")" );
                    }
                }
            }
        }
