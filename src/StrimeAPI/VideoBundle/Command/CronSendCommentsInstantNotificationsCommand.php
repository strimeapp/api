...
class CronSendCommentsInstantNotificationsCommand extends ContainerAwareCommand
{
    protected function configure()
    {
        $this
            ->setName('cron:video:send-comments-instant-notifications')
            ->setDescription('Send the instant notifications for comments')
        ;
    }

    protected function execute(InputInterface $input, OutputInterface $output)
    {
        $output->writeln( "[".date("Y-m-d H:i:s")."] CRON which sends instant notifications for comments" );
        ...
        if($comments != NULL) {
            ...
            // Get the list of comments in error
            $comments_in_error = $em->getRepository('StrimeAPIVideoBundle:CommentWithThumbInError')->findBy(array('nb_attempts' => 3));

            // Set a list of comments in error IDs
            $comments_in_error_ids = array();
            foreach ($comments_in_error as $error) {
                $comments_in_error_ids[] = $error->getComment()->getId();
            }
            ...
            foreach ($comments as $comment) {
                ...
                // And if the owner of the video wants to be notified right away
                if((strcmp($comment->getVideo()->getUser()->getMailNotification(), "now") == 0)
                    && (($comment->getUser() == NULL) || ($comment->getUser()->getId() != $comment->getVideo()->getUser()->getId()))) {
                    ...
                    // If the owner of the video has chosen to receive the notifications right away
                    // If the comment has a screenshot or is part of the thumbnails in error
                    if(($comment->getS3Url() != NULL) || (in_array($comment->getId(), $comments_in_error_ids))) {

                        // Add it to the list
                        $notifications[ $comment->getVideo()->getUser()->getSecretId() ][] = $comment->getSecretId();
                    }
                }
            ...
            foreach ($notifications as $user_id => $comments_list) {
                ...
                foreach ($comments_list as $comment) {
                    ...
                    // Set the endpoint
                    $endpoint = $strime_app_url."app/webhook/send-comments-right-away/".$user_id."/video/".$comment;
