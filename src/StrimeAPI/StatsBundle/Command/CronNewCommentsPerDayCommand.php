...
class CronNewCommentsPerDayCommand extends ContainerAwareCommand
{
    protected function configure()
    {
        $this
            ->setName('cron:stats:new-comments-per-day')
    ...
    protected function execute(InputInterface $input, OutputInterface $output)
    {
        ...
        if($stats == NULL) {
            ...
            $query = $em->createQueryBuilder();
            $query->select( 'api_stats_nb_comments.total_nb_comments' );
            $query->from( 'StrimeAPIStatsBundle:NbCommentsPerDay','api_stats_nb_comments' );
            $query->orderBy('api_stats_nb_comments.id', 'DESC');
