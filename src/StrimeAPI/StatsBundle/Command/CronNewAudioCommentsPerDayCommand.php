...
use StrimeAPI\StatsBundle\Entity\NbAudioCommentsPerDay;

class CronNewAudioCommentsPerDayCommand extends ContainerAwareCommand
{
    protected function configure()
    {
        $this
            ->setName('cron:stats:new-audio-comments-per-day')
            ->setDescription('Get the number of comments posted on audio files every day since a certain date')
        ;
    }
    ...
    protected function execute(InputInterface $input, OutputInterface $output)
    {
        ...
        $query->where('api_audio_comment.created_at >= :from_date');
        $query->andWhere('api_audio_comment.created_at < :to_date');
        ...
        $stats = new NbAudioCommentsPerDay;
        $stats = $em->getRepository('StrimeAPIStatsBundle:NbAudioCommentsPerDay')->findOneBy(array('date_time' => $date_time));

        if($stats == NULL) {

            // Get the stats of yesterday
            $query = $em->createQueryBuilder();
            $query->select( 'api_stats_nb_audio_comments.total_nb_comments' );
            $query->from( 'StrimeAPIStatsBundle:NbAudioCommentsPerDay','api_stats_nb_audio_comments' );
            $query->orderBy('api_stats_nb_audio_comments.id', 'DESC');
